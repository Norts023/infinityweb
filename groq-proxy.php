...
define('GROQ_API_KEY',      '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');          // ← substitua
...
// ─── Avisos ativos do painel Admin ────────────────────────────────────────────

$noticesPath = __DIR__ . '/data/notices.json';
$activeNotices = '';

if (file_exists($noticesPath)) {
    $allNotices = json_decode(file_get_contents($noticesPath), true) ?? [];
    $now = date('Y-m-d H:i:s');
    $active = array_filter($allNotices, function($n) use ($now) {
        if (empty($n['ativo'])) return false;
        if (!empty($n['expira_em']) && $n['expira_em'] < $now) return false;
        return true;
    });

    if (!empty($active)) {
        $lines = [];
        $prioLabel = ['alta' => 'ALTA', 'media' => 'MÉDIA', 'baixa' => 'BAIXA'];
        foreach ($active as $n) {
            $prio   = $prioLabel[$n['prioridade'] ?? 'media'] ?? 'MÉDIA';
            $tipo   = strtoupper($n['tipo'] ?? 'AVISO');
            $regiao = $n['regiao'] ?? 'Geral';
            $msg    = $n['mensagem'] ?? '';
            $desde  = $n['criado_em'] ?? '';
            $lines[] = "- [{$prio}] [{$tipo}] Região: {$regiao} | Desde: {$desde} | Mensagem: {$msg}";
        }
        $activeNotices = "\n\n⚠️ ALERTAS ATIVOS NO SISTEMA (mencione proativamente quando o cliente reclamar de problemas de conexão ou perguntar sobre o status da rede):\n" . implode("\n", $lines);
    }
}
...
- Se houver alertas ativos abaixo, use essas informações para responder clientes que reclamem de problemas.{$activeNotices}
PROMPT;
